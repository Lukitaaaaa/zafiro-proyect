...
let likeListenersAdded = false;

function setupLikeButtons(){
	// Solo agregar listener UNA VEZ usando delegación
	if (likeListenersAdded) {
		console.log('Like listeners already added, skipping');
		return;
	}

	// Delegación: escuchar clicks en document
	document.addEventListener('click', function(e) {
		// Like en posts
		const postLikeBtn = e.target.closest('.btn-like-toggle');
		if (postLikeBtn) {
			e.preventDefault();
		...
		// Like en comentarios
		const commentLikeBtn = e.target.closest('.btn-like-toggle--comment');
		if (commentLikeBtn) {
			e.preventDefault();
	...
	likeListenersAdded = true;
	console.log('Like listeners added with delegation');
}
